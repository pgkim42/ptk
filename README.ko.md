...
이름을 알 수 없어도 신뢰할 수 있지만, 모호한 결과, 조회 실패, listener 근거
...
시작, 다시 활성화, Settings 표시, 전달 전의 권한 상태 확인은 알림 권한 요청을
절대 띄우지 않습니다. macOS 권한 요청은 권한 상태가 미결정일 때 유효한 켜기
설정을 저장한 뒤에만 할 수 있습니다. 권한이 막히면 macOS Settings로 연결합니다.
...
- 저장된 감시 포트 프로필 빠른 전환
- 포트 변경 알림: opt-in 스위치, 선택 포트 표현식, macOS 권한 상태
