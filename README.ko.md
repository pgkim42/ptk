...
- CI는 Swift package tests, Swift build, release readiness, repository
  readiness checks를 실행합니다. 나중에 버전 태그를 올리면 unsigned
  산출물이 draft GitHub Release에만 올라가고 공개되지 않습니다.
  지금 `0.1.0` 준비 제목에 태그를 붙이라는 뜻은 아닙니다.
...
`0.1.0` 릴리스 준비에는 선택한 포트의 로컬 알림이 포함됩니다. 새로 설치하거나
