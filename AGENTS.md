...
## Project
- Name: `ptk`
- Goal: 개인용 AIO 상태바 도구 (첫 도구는 로컬 개발 포트 감시/정리)

## Structure
- `macos/`: Swift Package 기반 macOS 메뉴 막대 앱
  - `Sources/PTKApp/`: AppKit `NSStatusItem` 앱 셸 진입점
  - `Sources/PTKCore/Shell/`: AIO 상태바 앱 공통 로직
  - `Sources/PTKCore/Features/PortMonitor/`: 포트 파싱, 스캔, 프로세스 조회, 종료 안전 로직
  - `Sources/PTKCore/Features/ServiceMonitor/`: Docker daemon과 주요 로컬 DB 포트 상태 표시 로직
...
- 포트 기본 프로파일 변경 시 `README.md`, `macos/Sources/PTKCore/Features/PortMonitor/Domain/AppDefaults.swift`, 관련 테스트를 동기화한다.
